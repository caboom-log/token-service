...
class RefreshTokenServiceTest {
    ...
    void refreshToken() {
        ...
        Mockito.when(jwtTokenUtils.generateAccessToken(anyLong())).thenReturn("test_new_access_token");
    ...
    void refreshToken_Mismatch() {
        ...
        Assertions.assertThrows(InvalidTokenException.class,
                () -> refreshTokenService.refreshToken("test_expired_refresh_token"));
        Mockito.verify(jwtTokenUtils, Mockito.times(1)).isTokenValid(anyString());
        Mockito.verify(jwtTokenUtils, Mockito.times(0)).generateAccessToken(anyLong());
    }
}